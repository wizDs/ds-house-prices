...
num_ppl = Pipeline(steps=[('scaler', StandardScaler().set_output(transform="pandas")),
                          ('imputer', SimpleImputer(fill_value=-1).set_output(transform="pandas")),])
                          # TODO: Outlier detection

model = Pipeline(steps=[('preprocessor', num_ppl),
                        ('model', LinearRegression(n_jobs=-1))])

tt = TransformedTargetRegressor(regressor=LinearRegression(n_jobs=-1),
                                 func=np.log, inverse_func=np.exp)

model_log = Pipeline(steps=[('preprocessor', num_ppl),
                            ('model', tt)])
